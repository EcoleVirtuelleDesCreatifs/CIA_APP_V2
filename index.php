        ...
        <div class="row align-items-center hero-row">
            <div class="col-lg-6" data-aos="fade-right">
                ...
                <div class="hero-content">
                    ...
                    <h1 class="hero-title">Une force collective <span class="text-gradient animated-text" id="animatedWord">du financement</span> et de l'innovation en Afrique</h1>
...
<style>
    ...
    .animated-text::after {
        ...
        width: 100%;
        height: 3px;
        background: linear-gradient(90deg, #198754 0%, #1fa968 100%);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function typeText(text, index = 0) {
            if (index <= text.length) {
                animatedWord.textContent = text.substring(0, index);
                const delay = index === 0 ? 100 : 50 + Math.random() * 50; // Variable typing speed
                typingTimeout = setTimeout(() => typeText(text, index + 1), delay);
            } else {
                // Wait before deleting
                deletingTimeout = setTimeout(() => deleteText(text), 2500);
            }
        }
        ...
        function deleteText(text, index = text.length) {
            if (index >= 0) {
                animatedWord.textContent = text.substring(0, index);
                const delay = 30 + Math.random() * 30; // Faster deletion
                deletingTimeout = setTimeout(() => deleteText(text, index - 1), delay);
            } else {
                // Move to next phrase
                currentIndex = (currentIndex + 1) % phrases.length;
                typingTimeout = setTimeout(() => typeText(phrases[currentIndex]), 300);
            }
        }
